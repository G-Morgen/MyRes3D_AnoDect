...
# model = mobilenetv2.get_model( width_mult=0.45, num]_classes=600, sample_size = 112)#8
# model = shufflenet.get_model(groups=3, width_mult=2.0, num_classes=2)#9
...
# model = shufflenetv2.get_model( width_mult=2.0, num_classes=2, sample_size = 112)#14
# model = mobilenet.get_model( width_mult=2.0, num_classes=2, sample_size = 112)#15
# model = mobilenetv2.get_model( width_mult=1.0, num_classes=2, sample_size = 112)#16
model = squeezenet.get_model( version=1.1, num_classes=2, sample_size = 112, sample_duration = 8)
model = model.cuda()
model = nn.DataParallel(model, device_ids=None)	
print(model)

flops, prms = profile(model, input_size=(1, 1, 32, 112, 112))
print("Total number of FLOPs: ", flops/1000000)
