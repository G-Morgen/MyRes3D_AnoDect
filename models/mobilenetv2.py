...
class MobileNetV2(nn.Module):
    def __init__(self, num_classes=1000, sample_size=224, width_mult=1.):
        ...
        for t, c, n, s in interverted_residual_setting:
            ...
            for i in range(n):
                stride = s if i == 0 else (1,1,1)
                self.clf_features.append(block(input_channel, output_channel, stride, expand_ratio=t))
                input_channel = output_channel
        # building last several layers
        self.clf_features.append(conv_1x1x1_bn(input_channel, self.last_channel))
